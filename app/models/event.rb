class Event < ActiveRecord::Base
	...
	validates :start_time, :presence => {:message => 'cannot be blank, Event not saved'}
	
	validates :end_time, :presence => {:message => 'cannot be blank. Event not saved'}
	...
	def start_must_be_after_current_time
		errors.add(:start_time, "must be after current time") unless (self.start_time > Time.now)
	end 
